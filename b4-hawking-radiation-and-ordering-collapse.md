...
Two framework claims touch this contact.

### GB-2 — loss edge / ordering collapse
...
For the black-hole case, the relevant clause is the loss edge:
...
classical singularity
  ...
  → information-stranding pathology
```

### GB-3 / PB-4 — no null constitution / constitution at uptake
...
horizon:
  null/accessibility boundary
  transit / redshift / area-count / record-like site
...
singularity:
  ...
  geodesic termination
  invariant continuation failure
  source of the stranding threat in the classical story
...
area/count ≠ fine-grained constituted information
...
```text
information emitted by the singularity
```

It is not:

```text
the singularity transforming ordering directly into radiation
```

It is not:

```text
a null carrier constituting information in flight
```

And it is not, by itself:

```text
a proof that the horizon stores constituted information
```
...
```text
singularity:
  where classical GR's continuation fails
  where ordering-collapse lives
  where the information-loss pathology is sourced in the classical interior
...
```text
interior ordering appears to terminate
while exterior radiation appears thermal
```

Not because:

```text
the singularity emits thermal information
```

The framework's reading is therefore:

```text
GB-2 lives at the singularity / invariant edge.
PB-4 / GB-3 discipline lives at the radiation uptake and horizon/null-carrier boundary.
...
```text
Hawking radiation in flight:
  ordered influence / quantum field excitation structure
  no constituted information merely by null or near-null transit

Hawking radiation at uptake:
  detector clicks, collected quanta, exterior records
  flow-bearing constitution

Hawking radiation in fine-grained entropy accounting:
  radiation region R
...
```text
Hawking radiation carried a complete, ordinary exterior record with no need for island/interior correction,
while the singularity played no role in the loss pathology.
```

or if:

```text
information loss occurred without any invariant ordering-collapse / edge pathology.
```

The GB-3 / PB-4 discipline would fail if:

```text
Hawking quanta constituted information in null transit without flow-bearing uptake,
...
ordering destroyed vs transformed:
  classical GR says continuation fails;
  unitarity/Page curve favors transformed/accounted rather than destroyed

mechanism language:
  avoid saying ordering converts into Hawking radiation;
  that is too physical and not derived

horizon microstate readings:
  compatible only if they do not assign constituted information to null generators as such

final evaporation:
  live edge for whether the singularity/remnant/no-interior story changes the role map
...
Magnitude:
  externally priced
  framework-disowned
  no derivation of Hawking temperature, Page time, entropy coefficient, greybody factors, or evaporation rate
