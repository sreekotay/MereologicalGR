...
Flagged import: the Weyl decomposition and index-types are forced-from-ground (importable). The frame-adapted sharpness of the split, and the BKL wall-drive, are imported, not framework-derived. The new GB-2 falsifier (electric-Weyl signature on stranding) is forced only in the monotonic regime; in the generic regime it inherits the rotating wall-structure (S₃, the wall-symmetry — see the parent structural-seam, doc-1 §6.5).
...
5. Identification (forced remainder; entropic identification scoped) [re-scoped, promoted]: the spread is space-momentum with its ordering cancelled — **de-ordered** (directional cancellation) in the role vocabulary. Its measure is entropy **given** mixedness, an imported statistical premise, not forced here: a pure box (e.g. |k,−k⟩) has the de-ordered spread but S_vN = 0, so entropy tracks mixedness, not the spread itself. The de-ordered remainder is forced; its entropic reading scopes to thermal/typical radiation (the confirmation field below).‡
...
‡the syllogism fuses two senses the pure-state box separates: **de-ordered** (directional cancellation; present in the pure box) versus **disordered** (mixed; absent in the pure box). Entropy measures the latter. So entropy is not the measure of the de-ordered spread itself but of its mixedness — the remainder is forced, the entropic measure is mixedness-mediated (imported). [re-scoped, promoted]

[FIDELITY-RULE CANDIDATE (Rule 8, doc-3 — workspace register, cold-read gated; NOT yet applied): "de-ordered" / "space-momentum with its ordering cancelled" likely uses ordering's English connotation (directional) rather than its roster definition (causal precedence). What cancels in a resting box is adjacency-ORIENTATION — the signed spatial directions (the CPT P-assignment) — not ordering/precedence; momenta carry orientation, not precedence. If confirmed on cold read, the sweep is mechanical (de-ordered → de-oriented across this section, §9, and the doc-3 redshift/pure-state-box sites) and grades do not move (the remainder still forced; entropic reading still mixedness-scoped). Flagged here so the doc does not carry the candidate false-friend unmarked.]

EB-1 establishes the box (the whole — its mass). EB-2 asks what the mereology forces about its **interior** — a distinct mereological level, not a competing projection.

**Temperature:** T = ∂E/∂S = the slope relating the two faces (net/mass side to leftover/entropy side) — forced **as** A **relation** (the partition has a rate); its value (exponent d/a) is part-forced (a, the dispersion-power, by the part's flow→0) and part-imported (d, a dimensionless count). Scope [promoted]: T = ∂E/∂S is undefined where S = 0 (the pure-state box, S_vN = 0 — the probe's own case), so the temperature relation, like the entropic identification, scopes to the mixed/thermal regime.

**Hard-test** (survives; recorded so it can be re-run):
- *leftover forced?* Yes — photons have p≠0; box-at-rest has net p=0; so individual space-momenta must exist and cancel. No box-at-rest without leftover.
- *leftover **is** entropy (not "leftover momentum" by fiat)?* The chain to here is GR-obvious: a resting box of radiation has a timelike system four-momentum while its photons are null; the parts carry directed space-momentum, the resting whole carries none, so the directional content cancels at the whole. That whole/part gap is standard. The one step beyond GR is the measure: the de-ordered adjacency-spread is disorder in the role vocabulary, and entropy is how disorder is measured — so entropy is the measure the remainder calls for. Forced only if entropy-as-the-measure-of-disorder is a clean prior commitment, not here-convenient. (The cancellation is of spatial **direction**, plain isotropy — not the arrow; ordering is not the arrow.)
- *temperature forced?* As a relation — the cancellation partitions space-momentum into net (→mass) and leftover (→entropy); T is the partition's rate. Value part-imported.

**Grade: re-graded — world-exposure thin (per doc-3; the new evidentiary line):** the forced core is FAITHFULNESS — the de-ordered remainder is a GR round-trip (a resting radiation box has a timelike system four-momentum while its parts are null; the directional cancellation at the whole is standard). The one step beyond GR — identifying the remainder with entropy — is mixedness-scoped and IMPORTED (the pure-state box carries the remainder with S_vN = 0), and the grain finding (doc-3 interior-program) sharpens "given mixedness" to "given an uptake-history" (mixedness = trace over the flow-bearing thermalizer; thermality is uptake-constituted, not radiation self-reached). So EB-2 is not a clean world-register bet: forced core faithfulness + imported entropic measure. The earlier headline ("**bet** — you cannot have the box's mass without its entropy; one cancellation makes both") overstated — what one cancellation forces is the *remainder*; its *entropic reading* is imported. Confirmation field (thermodynamics) bears only on the imported half; the forced half is GR, zero world-weight.
- *could-have-failed (corrected — promoted):* the originally-stated guard was isotropy (the leftover could have been not de-ordered) — but isotropy was not the vulnerable clause; the box-at-rest is isotropic, so the remainder is de-ordered, forced. The actually-vulnerable clause is **mixedness**: the de-ordered spread is entropic only given mixedness (the pure-state box has the spread with S_vN = 0). The entropic identification is mixedness-guarded, not isotropy-guarded.
- *the bet is the **forcing*** (mass and entropy as two faces of one cancellation), **not** the Stefan-Boltzmann formulas — the exponent/coefficient mechanics (d/a, σ) are likely **echo** (kinetic theory has them), the vehicle not the bet.
...
Scalars are imputed from non-linear relationships, not imported as absolutes — GR's own practice, where curvature scalars are contractions: c from the quadrature, ℏ from conjugacy (Δp·Δx), T from E(S), σ from ℏ³c²k⁴. The framework **forces** the relationships; the relationships **impute** the scalars (relational, frame-independent — invariant because contracted), and only the unit-convention is free (it vanishes in natural units, c=ℏ=k=1). So "scale-free" means free of the unit-convention, **not** free of scales. GR-faithful (GR already does this); definition-grade. Distinct from EB-1/EB-2 — a general statement about scale, surfaced by the temperature derivation but standing on its own.

## 13. EB-1 / EB-2 Relation

EB-1 round-trips to GR (the box-has-mass); EB-2 decomposes that box's interior (the de-ordered remainder + its mixedness-scoped entropic reading). They are related — EB-2 decomposes EB-1's box — but **not** fused. Their grades, re-graded (§11): EB-1 a GR round-trip (faithfulness); EB-2 world-exposure-thin (forced core faithfulness + imported, mixedness-scoped entropic measure) — not the clean "thermo-bridging bet" earlier stated. Both sit largely in the faithfulness register; EB-2's only would-be world-content (the entropic identification) is the imported half.

---

## 14. References

Sources for this document are as cited in the parent framework — see doc-1 §20 (References) for the island/QES program (Almheiri-Engelhardt-Marolf-Maxfield; Penington), BKL (Belinskii-Khalatnikov-Lifshitz), cosmological billiards (Damour-Henneaux-Nicolai), and the timelike-entanglement-entropy literature. Cited here and not there: none — this document adds no sources beyond the parent's list and standard SR/GR (the §1 substrate). The CMB dipole (EB-1) and LIGO arm-length response (GB-3) are textbook observational facts, uncited by convention.
