...
different flow-worldline / frame-transport;
different rendered response temperature;
...
## 2. Standard Unruh machinery, role-located

Standard Unruh computation, schematically:
...
```text
temperature:
  flow-indexed energy scale of an accessible response / distribution

entropy:
  measure of accessible mixedness / multiplicity / uncertainty

information:
  ordered influence taken up in flow at application scale
```

Temperature can participate in information, but only after a write:

```text
thermal rendering / response function:
  pre-uptake structure
...
```text
Temperature is the flow-indexed response scale of accessible mixedness / relatedness;
...
```text
Minkowski vacuum:
  invariant phase-bearing relatedness / state input

inertial worldline:
  samples it as vacuum / no thermal detector bath

accelerated worldline:
  samples it through frame-transport and Rindler access structure

thermal response:
  temperature rendering of that relatedness under accelerated flow
...
```text
PB-4a sharpened:
  rendering is worldline / frame-transport set, observer-free
...
1. Analogue systems, cavity-enhanced setups, circuit QED, circular-motion detectors,
   and channeling experiments should favor worldline / access / coupling-dependent
   response models over readings that treat Unruh quanta as invariant in-flight particles
   or pre-constituted records.

2. Varying acceleration profile, detector coupling window, detector gap, switching
   function, or multi-detector arrangement should change the response in ways controlled
   by pullback / coupling / uptake, not by a detector-independent particle bath.
...
```text
flow-indexed:
  temperature requires a time parameter, worldline, congruence, modular flow,
  detector time, or application-equivalent evolution parameter.

scale parameter:
  temperature is not the microstate; it is the energy scale of a distribution or response.

accessible:
  temperature depends on what degrees of freedom are available to the renderer,
  subsystem, detector, or access algebra.
...
The Unruh case uses phase-bearing relatedness through field correlations and analytic / periodic structure under accelerated pullback. It does not reduce the whole phenomenon to a single U(1) phase.
...
4. **Calibration is allowed.**  
   Constants, normalizations, couplings, thresholds, detector gaps, and apparatus-specific rates may be externally supplied or empirically calibrated if the role-route / form was fixed first.
