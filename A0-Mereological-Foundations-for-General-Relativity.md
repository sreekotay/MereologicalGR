...
This sharpens "render-only" from "flow = 0, no forced face" to "boost fixed-ray, touched by inter-frame transport only through its eigenvalue." It also closes a loop toward the scalar-response run: the forced face's natural horizon (the boost-Killing / Rindler horizon) is itself null, generated by these fixed rays — the locus where the render-only field content an accelerated worldline samples lives. The Unruh number stays outside A0; only the placement is fixed here.

Discipline: redshift is not derived here — it **is** the boost eigenvalue; A0 only locates which role-slot it occupies (render-only content, scaled by the forced-face eigenvalue).
...
This matters because MGR often names things that are not all the same kind:
...
Note, e.g., that we place "photon" and "quantum entanglement" as adjacent in this work - a mereology of roles, not substances.
...
Mass matters because uptake requires flow-bearing capacity. A null or in-flight content can be consequence-capable without already being information. For content to become information, it must be taken up into a flow-bearing register and cross an application consequence threshold.

Candidate operational reading:

```text
mass is the usual physical mark that content has, or can enter, a flow-bearing register.
```

This is not a new primitive. It is a projection-side bridge between rendered content and uptake.
...
mass:
  projection / flow-face of energy-momentum

temperature:
  response scale / projection parameter

record:
  application-level stabilized state

constitution:
  completed uptake at threshold
```

Promote nothing to peer-role unless compose / missing / extra forces it.

**The companion rule: anti-over-fusing (the separability / corner-strip test).**
...
guarded by the companion:
  fusing two roles because they share a dependency
  (over-fusing)
```

The test for any fusion claim is the corner strip:
...
This is not a new instrument. It is the move the corpus already runs, now named:
...
```text
shared dependency:
  acceleration needs flow;
  uptake needs flow

not an identity:
  free-fall: a = 0, uptake intact
  → acceleration and uptake separable
```
