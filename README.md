...
```text
MGR audits the role-composition inside GR's successful description. GR is held fixed. The work asks which roles GR can successfully leave bundled, which become clearer when unfused for export, and which should remain bundled.

This is a foundations workbench.

The photon opens the carve: ordering and influence remain while proper-time flow is stripped.
Claims are graded.
Some bets are expected to fail or demote.

Chronology is not evidence.
Lineage is the criterion: the role-route must be fixed before contact.
...
|---|---|---|
| **ordering** | Causal / light-cone structure (imported from GR) | MGR novelty claim |
...
The stake is forward. A description defused at export and one axiom-fit after agree on every measurement in hand by construction — the past cannot discriminate them. They differ in what they force next, and that difference must be pre-registered as exposure. Unruh thermality re-read as a lock between the relatedness and influence legs forces per-sector questions the fused "the vacuum is thermal" cannot phrase (`c8`); virtual photons re-read as unconverted relatedness force a width-not-tilt discipline the fitted closure never states (`b1`, `c8`).
...
c7-the-third-order          the relatedness cone; forced tier classification (not canon)
...
d2-the-abundance-arithmetic occupancy: the 5.4 ratio priced by production branch (not canon; DR-4 fork)
diagnostic-ledger.md      lane index (§4)
contrast-diagnostic-program.md
archive/b5-torsion-*      sensitivity workbench only (D9)
workbench/, data/           numeric experiments
```

Detail and falsifiers: **CLAIMS.md**, then the cited source file.