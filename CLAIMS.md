...
| **DR-4** | The abundance fossil: Ω_dark/Ω_b ≈ 5.4 is evidence about interface history, not a parameter. Computed (d2): strict seam-count-one at all epochs cannot make 5.4 a prediction — O(1) branching is excluded outright (the hot-relic ceiling: abundance-matched relics go non-relativistic at T = 1.16 eV/Br, equality itself at Br = 1); the only natural branch is a dark sector running its own asymmetry, which requires asymmetric reheating (T′/T < 0.45) and generically leaves **ΔN_eff ≳ 0.05** | CMB-S4 (σ(N_eff) ≈ 0.03); tensor modes police the accident branch (PIDM needs high-scale inflation) | fork, pre-registered: ΔN_eff detected near 0.05 → the fossil shows (early interface, closed); hard null → the lane retreats to the accident branch and the 5.4 discount locks permanently | a non-gravitational detection (inherited kill); seam death strands with the lane | d2 |
