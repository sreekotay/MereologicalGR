...
Exposure rows for the C-lane (c1–c8). Placed July 2026 with **one event in hand** (GW170817),
...
| **CD-6** | Axis: any resolved cross-leg anisotropy points at one of two pre-registered apexes — the CMB dipole apex, galactic (l, b) ≈ (264°, 48°), or the matter-bulk apex ~28° away (c1's congruence fork; the grammar leans matter-side) — and a detection selects the constituting congruence. *Forked 2026-07 to match the fork registered in c1 and CD-9's two-template design; no cross-leg anisotropy data exists, so this is reconciliation with a registered fork, not adjustment after contact* | far-future population (N ~ 10³–10⁵) | registered; untestable soon | resolved axis at neither apex | c1, c3 |
| **CD-7** | The split: BBN-side-only N_eff shifts are the family's residue register (12.3 κ ε_max); equal-shift anomalies are dark radiation; CMB-side-only are neither. Sign computed: composite/flow (b7-licensed) gives κ = +1/2 — deficits bound, don't feed. **Status 2026-07:** LBT Y_p (arXiv:2601.22238) supersedes EMPRESS, anchoring the BBN side standard — κε_max = −0.004 ± 0.006, ε_max ≤ 0.014; the residual anomaly migrated to the CMB side (2.81 ± 0.12, foreground-suspect), numerically near the old target and **rejected by the sidedness rule** — the register refusing a wrong-side match is the discipline working. Band closes: excluded-unsaturated p < 0.78 vs 0.77 visibility; survivors p < 0.77 (silent) or p > 0.89 (saturated) | helium-systematics resolution (EMPRESS vs LBT vs Aver); CMB-S4; d(p,γ)³He | dormant bound; the fusion measured at ΔH/H = −0.4 ± 0.6% (BBN epoch) | BBN-side excess above ε_max (kills the composite class); CMB-side or equal-shift anomaly claimed as cone currency | c4, c6 |
