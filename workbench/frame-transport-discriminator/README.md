...
FT-2:
  proper acceleration is one local face of frame-transport.

FT-3:
  Unruh scalar response can be role-located through frame-transport / rendering,
  rather than only worldline pullback.
```
...
No row here is a direct scalar Unruh detector test.
```

Files:

```text
frame_transport_handles.csv:
  source-linked external measurement / handle table.

numeric_residuals.csv:
