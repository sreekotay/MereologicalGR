# Diagnostic Ledger — Contact Lanes, Residual Guards, and Route Tests
...
The project now distinguishes five kinds of outward contact.

```text
route diagnostic:
  existing machinery should enter through the role route named by the framework
...
formal discriminator:
  a theoretical program may require or fail to require an otherwise forbidden term
...
residual / guard:
  what extra term is forbidden, allowed, or source-routed

status:
  current qualitative state

failure / demotion condition:
  what would actually break or weaken the role claim
```
...
where pullback, coupling, frame-transport, and uptake models succeed.
```
...
```text
response should route through pullback / coupling / switching / detector gap / uptake,
...
```text
Page-QES can change recoverability/accounting.
It becomes MGR-engine-eligible only if it changes the write-site or causal access at write,
not merely the entropy bookkeeping before uptake.
```

Failure / demotion condition:

```text
a black-hole observable or formal term requires constituted information on null structure
while exterior uptake/write is fixed,
and the term cannot be reduced to area, edge, reconstruction, bulk entropy,
backreaction, or non-null microstructure.
```
...
Framework expectation, low weight:

```text
extra structure should be geometry/reconstruction/backreaction-priced,
not constitution-priced.
```

This is a discriminator, not a prediction of what holography will discover next.
...
CMB temperature-redshift guard:

```text
standard lane:
  T_CMB(z) = T0(1+z), with known ways to parameterize deviations
...
Information-specific redshift guard:
...
```text
Unruh coefficient;
Einstein-Cartan / Poincare-gauge torsion coupling;
...
Most rows are therefore one of four low-weight forms:

```text
null guard:
  forbidden residual stays absent

route diagnostic:
  existing machinery enters through the correct layer
...
```text
This is not a list of predictions; it is a map of contact lanes where the role grammar
could be stressed, bounded, demoted, or externally priced.
```