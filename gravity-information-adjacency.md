...
- GB-2: information-stranding-at-an-edge ⟺ ordering-collapse (and only there). [biconditional]
  ...
  - "only there": the cone gives exactly two edge-types, each forcing its pathology; a geodesically-complete cosmology (bounce) has no edges, hence no stranding-pathology.
...
- GB-3, horizon test (the new-scope domain): the sharp check is whether black-hole horizon entropy (S=A/4) is constituted information or record-structure. Verdict: STANDS, on the physics' own coarse/fine distinction. S=A/4 is coarse-grained — the log of the number of microstates consistent with the macrostate (a count) = record by the framework's criterion. The fine-grained, constituted information (the Page curve via the island/QES formula) is recovered in the radiation R (flow-bearing quanta, collected by an external observer) and the island (interior matter), with the horizon contributing only the area term (the count), not the constituted information. So the null horizon carries record; constitution is at the flow-bearing radiation/matter — exactly GB-3. Could-have-failed: if the island construction had localized the fine-grained information on the null horizon, GB-3 fails; instead it sits in R and the island. Residual: a minority reading (horizon-microstate geometries / thin shell of entangled quanta just under the horizon) — but that shell is flow-bearing quanta, not the null generators, so it too is consistent with GB-3. The reading that would break GB-3 (constituted information on the null generators themselves, no flow-bearing carrier) was not found.
- GB-1, influence-identity test: run, passes at the role level. Both gravity's influence (energy-momentum constraining causal structure) and information's influence (correlation constraining a partner) are constraint-relations satisfying the definition; the apparent differences are carried elsewhere, not by influence — transferring-vs-non-transferring by the energy-momentum term (third term), universal-vs-selective by the selection criterion (a configuration of the one role). Could-have-failed: if information's correlation-structure were an extra constituent rather than a configuration, GB-1 fails; it came out configuration. Verdict: stands at the role level; residual exposure reduced to the bare GB-1 falsifier (a system that gravitates while information-like with no adjacency-difference, or carries information with gravity's full adjacency sector).
...
- Perimeter: the role-decomposition places the terms and the conversion factor c; it does not derive the combining-law (quadrature E² = (pc)² + (mc²)²) or the signature/count (1 timelike, 3 spacelike). The quadrature is imported but required — it is what makes the flow-crossing a conserved conversion. A partial-relabel standing on imported structure, not a numeric derivation.
...
- the four-momentum's two legs (named by projection onto the laying-out roles): time-momentum = the (ordering+flow)-projection (timelike part; magnitude from flow; zero when flow→0 — why the photon is massless); space-momentum = the (ordering+adjacency)-projection (spacelike part; ordinary momentum).
- mass = the invariant MAGNITUDE of the four-momentum (rest value), NOT a third conjugate.
- ordering has NO conjugate-quantity, a CATEGORY FACT not a gap: ordering is the shared SPINE of both legs (in time and in space), the axis the legs project ALONG, not a leg. Asking ordering's conjugate quantity is a category error. This is why c's units have ordering cancel (c = space/time = (ordering+adjacency)/(ordering+flow) = adjacency/flow along the spine — leg/leg, spine cancels); the photon is the spine made manifest, c saturated.

Ordering and influence are CONJUNCTS (cause = their conjunction; they degrade separately into sequence and correlation), not conjugates (they do not trade off) — and ordering, being the spine not a leg, cannot host a leg-shell at all. So the ordering axis carries a ratio-bound (s = influence/ordering, measured AGAINST the spine — ordering in the denominator, not cancelling) but no mass-shell. c (leg/leg along the spine, spine cancels) and s (influence against the spine, spine present) are NOT exact peers — the c/s asymmetry, grounded.

Gravity's content is the four-momentum (time-momentum + space-momentum); information uses the ordered content (cause + flow); entropy is the measure of space-momentum once its ordering cancels — the de-ordered adjacency-spread, which is disorder in the role vocabulary, and entropy is how disorder is measured. (Mass, the invariant leg, carries no entropy — it is not a spread.)
...
5. Therefore: by the quadrature (net p=0 → E=mc²), the energy lands on the time-momentum leg → the box has rest mass.
6. So: the photon-at-rest *is* the box-at-rest — carrying the mass, rest, and rest-frame the part could not.
7. Witness: the CMB — a real photon aggregate with a rest frame (measurable via the dipole) and mass. The forced chain, instanced in the world.
...
**Grade:** round-trips to GR (box-has-mass is GR). [The content-selected-rest framing is separate — TBD whether it carries its own bet-content.]
...
**The claim:** the same cancellation that gives the photon-box its mass forces a remainder — an isotropic, de-ordered adjacency-spread — and that remainder is what the framework already calls entropy. Mass and entropy, two faces of one forced cancellation.
...
5. Identification (forced): the spread is space-momentum with its ordering cancelled — disorder in the role vocabulary — and entropy is its measure.‡
...
‡not a new component added to the box: entropy is how disorder is measured, and the de-ordered adjacency-spread is disorder — so entropy is the measure the remainder already calls for.
...
**Temperature:** T = ∂E/∂S = the slope relating the two faces (net/mass side to leftover/entropy side) — forced AS A RELATION (the partition has a rate); its value (exponent d/a) is part-forced (a, the dispersion-power, by the part's flow→0) and part-imported (d, a dimensionless count).
...
**Grade: BET** — given the box, entropy is forced as the leftover of the mass-cancellation. Sharper than "GR-continuous bridge to thermo": you cannot have the box's mass without its entropy — one cancellation makes both. Diverges from GR (GR-silent on entropy/temperature); confirmation field is thermodynamics.
- *could-have-failed:* the leftover could have been not de-ordered (then not entropy); but the box-at-rest is isotropic, so it is de-ordered. Forced.
